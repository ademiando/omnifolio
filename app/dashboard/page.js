...
function formatCurrencyCompact(value, valueIsUSD, displaySymbol, usdIdr) {
    ...
    const isRupiah = displaySymbol !== '$';
    const sign = noSym.startsWith('-') ? '-' : '';
    const num = noSym.replace('-', '');
    return `${sign}${isRupiah ? 'Rp ' : '$'}${num}`;
}
...
const Modal = ({ children, isOpen, onClose, title, size = "2xl" }) => {
  ...
  return (
    ...
    >
      <div className={`glass-card w-full ${sizeClasses[size]} max-h-[85vh] flex flex-col transform transition-all duration-300 scale-100 my-auto shadow-2xl relative`} onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center p-4 border-b border-white/10 shrink-0 bg-black/30 rounded-t-xl">
          <h2 className="text-lg font-semibold text-white">{title}</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-white text-2xl px-2 py-1 leading-none rounded hover:bg-white/10 transition-colors focus:outline-none">&times;</button>
        </div>
        <div className="p-4 overflow-y-auto flex-1 scrollbar-thin relative">
          {children}
        </div>
      </div>
    </div>
...
const BottomSheet = ({ isOpen, onClose, children }) => {
  useEffect(() => {
    ...
    if (isOpen && isBrowser) document.addEventListener('keydown', handleKeyDown);
    return () => { if (isBrowser) document.removeEventListener('keydown', handleKeyDown); };
  }, [isOpen]);
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        if (searchMode === 'crypto') {
            ...
            const formatYahooData = (data) => {
                ...
                return data.quotes
                    ...
                    .map(it => {
                        ...
                        return { symbol: it.symbol, display: `${it.shortname || it.longname || it.symbol} (${it.symbol})`, exchange: it.exchDisp || it.exchange, id: it.symbol, source: "yahoo", type: "stock", image: imgUrl };
                    });
            };

            const formatTVData = (data) => {
                 if (!data || !data.length) return [];
                 return data.map(it => {
                    const ySym = getYahooSymbol(it.symbol, it.exchange, it.country);
                    const isIndo = ySym.endsWith('.JK');
                    const cleanSymbol = ySym.replace('.JK', '');
                    const imgUrl = it.logoid 
                        ? `https://s3-symbol-logo.tradingview.com/${it.logoid}--big.svg`
                        : (isIndo ? `https://assets.stockbit.com/logos/companies/${cleanSymbol}.png` : `https://assets.parqet.com/logos/symbol/${ySym}?format=png`);
                    return { symbol: ySym, display: `${it.description} (${ySym})`, exchange: it.exchange || it.country, id: ySym, source: "tradingview", type: "stock", image: imgUrl };
                });
            };
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        console.error("Semua server pencarian sedang sibuk atau diblokir.");
                    }
                }
            }
          ...
          console.error("Pencarian error:", e); setSuggestions([]); 
      } finally { 
    ...
    return (
        <div className="p-2 sm:p-4 text-gray-300 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div>
                     <h3 className="text-sm text-gray-500 font-medium mb-3 flex items-center gap-2">Top Gainers</h3>
                     <div className="space-y-1">
                         {topGainers.map(g => {
                 ...
                 <div>
                     <h3 className="text-sm text-gray-500 font-medium mb-3 flex items-center gap-2">Top Losers</h3>
                     <div className="space-y-1">
                         {topLosers.map(g => {
                             const assetMatch = assets.find(a => a.symbol === g.symbol);
                             const imgSrc = assetMatch?.image || g.image || `https://ui-avatars.com/api/?name=${g.symbol}&background=random&color=fff&rounded=true&bold=true`;
...
const AddAssetForm = ({ searchMode, setSearchMode, query, setQuery, suggestions, setSuggestions, setSelectedSuggestion, isSearching, setIsSearching, addAssetWithInitial, addNonLiquidAsset, nlName, setNlName, nlQty, setNlQty, nlPrice, setNlPrice, nlPriceCcy, setNlPriceCcy, nlPurchaseDate, setNlPurchaseDate, nlCoupon, setNlCoupon, nlYoy, setNlYoy, nlDesc, setNlDesc, nlIncomeFreq, setNlIncomeFreq, displaySymbol, handleSetWatchedAsset, watchedAssets }) => {
    ...
    return ( <div className="space-y-4"> <div className="flex border-b border-white/10">{[{ key: 'stock', label: 'Stock' }, { key:'crypto', label:'Crypto' }, { key:'nonliquid', label: 'Alternative Assets' }].map(item => (<button key={item.key} onClick={() => setSearchMode(item.key)} className={`px-3 py-2 text-sm font-medium ${searchMode === item.key ? 'text-white border-b-2 border-emerald-400' : 'text-gray-400'}`}>{item.label}</button>))}</div> {searchMode !== 'nonliquid' ? ( <div className="space-y-4"> 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 relative z-10"><div><label className="text-xs text-gray-400">Qty</label><input value={shares} onChange={e => handleInputChange('shares', e.target.value)} className="w-full mt-1 rounded bg-zinc-800 px-3 py-2 text-sm border border-zinc-700 text-white" type="text" /></div><div><label className="text-xs text-gray-400">Price ({displaySymbol})</label><input value={price} onChange={e => handleInputChange('price', e.target.value)} className="w-full mt-1 rounded bg-zinc-800 px-3 py-2 text-sm border border-zinc-700 text-white" type="text" /></div></div> <div className="relative z-10"><label className="text-xs text-gray-400">Total Value ({displaySymbol})</label><input value={total} onChange={e => handleInputChange('total', e.target.value)} className="w-full mt-1 rounded bg-zinc-800 px-3 py-2 text-sm border border-zinc-700 text-white" type="text" /></div> <div className="flex justify-end relative z-10"><button onClick={() => addAssetWithInitial(shares, price)} className="bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2 rounded font-semibold">Add Position</button></div> </div> ) : ( 
...
const PortfolioAllocation = ({ data: fullAssetData, tradingBalance, displaySymbol, usdIdr }) => {
    ...
    const { equityData, sectorData } = useMemo(() => {
        const tradingBalanceUSD = tradingBalance / (usdIdr || 16200);
        
        const greens = ["#6ee7b7", "#34d399", "#10b981", "#059669", "#047857", "#064e3b"]; 
        const blues = ["#93c5fd", "#60a5fa", "#3b82f6", "#2563eb", "#1d4ed8", "#1e40af"];

        const secDataMap = {
            'Cash': { value: tradingBalanceUSD || 0, color: '#1e40af', icon: <WalletIcon className="w-4 h-4 sm:w-5 sm:h-5 text-blue-400" /> },
        ...
        (fullAssetData || []).forEach(asset => { 
            if (asset.type === 'stock') secDataMap['Equity'].value += (asset.marketValueUSD || 0); 
            else if (asset.type === 'crypto') secDataMap['Crypto'].value += (asset.marketValueUSD || 0); 
            else if (asset.type === 'nonliquid') secDataMap['Alternative Assets'].value += (asset.marketValueUSD || 0); 
        }); 
        
        const finalSectorData = Object.entries(secDataMap).map(([name, data]) => ({ name, ...data })).filter(d => (d.value || 0) > 0.01);

        const allAssets = [
            ...(fullAssetData || []).map(d => ({ name: d.symbol, value: d.marketValueUSD || 0, image: d.image, type: d.type })),
            { name: 'Cash', value: tradingBalanceUSD || 0, image: null, type: 'cash' }
        ].filter(a => (a.value || 0) > 0.01).sort((a, b) => b.value - a.value);

        let finalEquityData;
        if (allAssets.length > 12) {
            const top11 = allAssets.slice(0, 11);
            const others = allAssets.slice(11);
            const othersValue = others.reduce((sum, item) => sum + (item.value || 0), 0);
        ...
        finalEquityData = finalEquityData.map((d) => {
            ...
            if (d.type === 'cash') {
                ...
                if (assetIndex < half) { color = greens[Math.min(assetIndex, greens.length - 1)]; } 
                else { color = blues[Math.min(assetIndex - half, blues.length - 1)]; }
                assetIndex++;
            }
            return { ...d, icon, color };
        });
    ...
    const totalValueUSD = useMemo(() => data.reduce((s, d) => s + (d.value || 0), 0), [data]);

    if (!totalValueUSD || totalValueUSD <= 0 || isNaN(totalValueUSD)) {
        return <div className="mt-8 text-center text-gray-500">No assets to show in allocation.</div>;
    }

    const totalValueDisplay = totalValueUSD * (displaySymbol === "Rp" ? usdIdr : 1); 
    const size = 200, strokeWidth = 20, innerRadius = Math.max((size / 2) - strokeWidth, 1); 
